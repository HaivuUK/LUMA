...
- Automate what you can, leave choice to users only if you cannot choose for them.
...
Generally, the approach should try to follow the 'local' coding practices, that is, new code should not look completely out-of-place from it's context. Beyond that, we would like to see these guidelines followed as much as they don't conflict with the file being edited:
...
Summarising, the typical flow for getting changes merged runs like this:
...
- And all contributions should be fully compliant with the AGPLv3 or compatible open source licenses we use!
